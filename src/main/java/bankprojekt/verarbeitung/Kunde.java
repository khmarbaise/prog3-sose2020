...
//import-Anweisungen bieten genaugenommen nur eine Abk�rzung. Sie 
//erlauben es, im Code nur den unqualifizierten KLassennamen zu
//verwenden, statt �berall das komplette Paket mitschreiben zu m�ssen.
//java.time.LocalDate heute = java.time.LocalDate.now();
//ist doch viel schwerer lesbar als
//LocalDate heute = LocalDate.now();
//Das Paket java.lang mit seinen Klassen wird automatisch importiert.
...
public class Kunde implements Comparable<Kunde>{
	...
	//Eine statische (static) Konstante (final). Statisch bedeutet, dass es
	//diesen Musterkunden nur ein einziges Mal im Programm gibt, nicht f�r
	//jedes einzelne Kunde-Objekt neu. Man greift �ber den Klassennamen
	//darauf zu, also Kunde.MUSTERMANN.
	//Namenskonvention: statische Konstanten komplett in Gro�buchstaben
	...
	public static String getANREDE() {
		//statische Zugriffsmethode. Auch statische Methoden werden �ber
		//den Klassennamen aufgerufen, nicht �ber ein Objekt (obwohl
		//das m�glich w�re, aber das ist saum��ig schlechter Stil...)
		return ANREDE;
	}
	...
	/**
	 * der Destruktor der Klasse. Er wird "irgendwann" aufgerufen,
	 * wenn der Garbage Collector ein Objekt aus dem Speicher entfernt.
	 * Wann dies genau sein wird und ob es �berhaupt in einem Programm
	 * geschieht, ist unvorhersagbar. Deshalb wird die Verwendung der
	 * finalize-Methode nicht (mehr) empfohlen. Stattdessen kann die
	 * folgende Klasse verwendet werden.
	 */
	 ...
	 * 
	 * Hier macht sie nur eine Ausgabe, weil in diesem Beispiel
	 * einfach keine Aufr�umarbeiten zu erledigen sind. Es w�re hier aber
	 * m�glich, offene Verbindungen zu Datenbanken/Dateien/Netzwerken
	 * zu schlie�en oder auch die Daten des Objektes noch
	 * irgendwo zu speichern.
	 * 
	 * Diese Klasse soll Sie an die verwendung von Interfaces erinnern.
	 * Der Konstruktor der Klasse Thread erwartet n�mlich ein Objekt,
	 * das das Interface Runnable implementiert. Also tut diese Klasse das.
	 * 
	 * Ja, es gibt private innere Klassen, also Klassen, die innerhalb
	 * von anderen Klassen definiert sind. Wir werden uns hier nicht
	 * ausf�hrlich damit besch�ftigen. Wichtig ist: Man hat in der
	 * inneren Klasse Zugriff auf alle Attribute und Methoden der 
	 * �u�eren Klasse, auch auf die privaten.
	 * 
	 * @author Doro
	 *
	 */
	...
	{
		/**
		 * In der Methode run steht, was passieren soll, wenn das
